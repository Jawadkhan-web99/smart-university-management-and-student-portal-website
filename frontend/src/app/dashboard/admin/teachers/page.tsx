...
import { TeacherProfileData, DepartmentItem, CourseItem } from '../../../../types/university';
import {
  UserCheck,
  Search,
  Plus,
...
interface ExtendedTeacherProfile extends TeacherProfileData {
  courses?: Array<{
    _id: string;
    courseCode: string;
    title: string;
...
export default function AdminTeachersPage() {
  const [teachers, setTeachers] = useState<ExtendedTeacherProfile[]>([]);
  ...
  const [activeTeacher, setActiveTeacher] = useState<ExtendedTeacherProfile | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    const res = await apiFetch<{
      teachers: ExtendedTeacherProfile[];
      total: number;
      totalPages: number;
  ...
  const openEditModal = (teacher: ExtendedTeacherProfile) => {
    ...
    setFormData({
      ...
      isActive: Boolean(teacher.user?.isActive),
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setFormError(res.message || 'Failed to onboard faculty member');
    }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    const res = await apiFetch(`/admin/teachers/${activeTeacher._id}`, {
      method: 'PUT',
      body: JSON.stringify(formData),
    });
  ...
  const toggleCourseAllocation = (courseId: string) => {
    setFormData((prev) => ({
      ...prev,
  ...
  return (
    <ProtectedRoute allowedRoles={['admin']}>
      <DashboardShell role="admin" title="Faculty & Teaching Staff Directory">
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                Manage instructional personnel, academic designations, and course teaching allocations.
              </p>
            </div>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-md transition-colors self-start sm:self-auto"
            >
          ...
          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col md:flex-row items-center gap-3">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-white outline-hidden focus:ring-2 focus:ring-indigo-500"
              />
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
            {loading ? (
              <div className="py-20 flex flex-col items-center justify-center space-y-3">
                ...
                <p className="text-xs text-slate-500">Retrieving faculty roster...</p>
              </div>
            ) : teachers.length === 0 ? (
              <div className="py-16 text-center space-y-2">
                ...
                <p className="text-xs text-slate-500">
                  Onboard a new professor or adjust your search filter.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200/80 dark:border-slate-800 text-slate-500 uppercase tracking-wider font-semibold">
                    <tr>
                      ...
                      <th className="px-4 py-3.5">Courses Assigned</th>
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          <div className="flex flex-wrap gap-1">
                            {tc.courses && tc.courses.length > 0 ? (
                              tc.courses.map((c) => (
                                <span
                                  key={c._id}
                                  className="px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 font-semibold text-[10px]"
                                >
                                  {c.courseCode}
                                </span>
                              ))
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="inline-flex items-center gap-1">
                            ...
                            <button
                              ...
                              title="Edit teacher & allocations"
                            >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        type="email"
                        required
                        placeholder="faculty@apex.edu.pk"
          ...
          {/* EDIT TEACHER MODAL WITH COURSE ALLOCATIONS */}
          {showEditModal && activeTeacher && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
              <div className="w-full max-w-xl rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">
                    Edit Faculty & Course Allocations
                  </h3>
                  <button
                    type="button"
                ...
                <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
                  ...
                  {/* Course Allocations Multi-Select */}
                  <div className="space-y-1.5 pt-1">
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                      Allocate Teaching Courses ({formData.courses.length} selected)
                    </label>
                    <div className="max-h-36 overflow-y-auto p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 space-y-1">
                      {allCourses.length === 0 ? (
                        <p className="text-[11px] text-slate-400 p-2">No courses available.</p>
                      ) : (
                        allCourses.map((c) => (
                          <label
                            key={c._id}
                            className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-800 text-xs text-slate-700 dark:text-slate-200 cursor-pointer"
                          >
                            <input
                              type="checkbox"
                              checked={formData.courses.includes(c._id)}
                              onChange={() => toggleCourseAllocation(c._id)}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <input
                      type="checkbox"
                      id="editTeacherActive"
                      checked={formData.isActive}
                      onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })}
                      className="rounded-sm border-slate-300 text-indigo-600 focus:ring-indigo-500"
                    />
                    <label
                      htmlFor="editTeacherActive"
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
              <div className="w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-5 animate-in fade-in zoom-in-95">
                <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-black text-base">
                      {activeTeacher.user?.firstName?.[0] || 'T'}
                    </div>
                    <div>
                      <h3 className="text-base font-extrabold text-slate-900 dark:text-white">
                        {activeTeacher.user?.firstName} {activeTeacher.user?.lastName}
                      </h3>
                      <p className="text-xs text-slate-400 font-mono">{activeTeacher.teacherId}</p>
                    </div>
                  </div>
                  <button
                    type="button"
                ...
                <div className="space-y-2.5 text-xs">
                  ...
                  <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60">
                    ...
                    <span className="font-bold text-slate-800 dark:text-slate-200">
                      {activeTeacher.department?.name || 'Unassigned'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60">
                    <span className="text-slate-400">Email</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">
                      {activeTeacher.user?.email}
                    </span>
                  </div>
                  <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60">
                    <span className="text-slate-400">Qualification</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">
                      {activeTeacher.qualification || 'N/A'}
                    </span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {activeTeacher.courses && activeTeacher.courses.length > 0 ? (
                        activeTeacher.courses.map((c) => (
                          <span
                            key={c._id}
                            className="px-2 py-0.5 rounded-md bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 font-mono text-[10px]"
                          >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
              <div className="w-full max-w-sm rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-4 animate-in fade-in zoom-in-95">
                ...
                <div className="text-center space-y-1">
                  ...
                  <p className="text-xs text-slate-500">
                    Are you sure you wish to delete faculty record for{' '}
                    <strong className="text-slate-900 dark:text-white">
                      {activeTeacher.user?.firstName} {activeTeacher.user?.lastName}
                    </strong>
                    ? This action will unassign any active courses.
                  </p>
                </div>
